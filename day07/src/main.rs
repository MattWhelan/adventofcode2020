...
impl FromStr for Rule {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let mut parts = s.splitn(2, "contain ");
...
fn main() -> Result<()>{
    ...
    let contents = count_bags("shiny gold", &rule_map);

    println!("Shiny Gold Bag contains {} others", contents);
    Ok(())
}
...
fn search<'a>(needle: &str, haystack: &'a [(String, String)]) -> HashSet<&'a str> {
    ...
    for color in ret.iter() {
        acc.extend(search(color, haystack));
    }
    acc.extend(ret);
    acc
}

const INPUT: &str = r#"posh brown bags contain 5 dim coral bags, 1 plaid blue bag, 2 faded bronze bags, 2 light black bags.
