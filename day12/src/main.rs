...
impl Nav for Ship {
    ...
    fn forward(&mut self, dist: isize) {
        self.position = self.position + self.heading * dist;
    }
...
impl FromStr for Maneuver {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let num: isize = chars.as_str().parse()?;

        let ret = match ins {
            'N' => Maneuver::DIR(NORTH * num),
            'E' => Maneuver::DIR(EAST * num),
            'S' => Maneuver::DIR(SOUTH * num),
            'W' => Maneuver::DIR(WEST * num),
            'R' => Maneuver::TURN(num),
            'L' => Maneuver::TURN(-num),
            'F' => Maneuver::FWD(num),
